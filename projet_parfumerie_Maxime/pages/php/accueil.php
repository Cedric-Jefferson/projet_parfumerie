...
<div class="row">
		...
		if( isset( $_POST['panier']) ){
			...
			$Quer1 ="INSERT INTO `commande`(`id_comm`, `date`, `montant`, `id_produit`, `num_adherent`,`quantite`) VALUES (default,CURRENT_DATE,$_POST[prix],'$_POST[num_produit]',$_SESSION[num_adherent],1); ";
			$Resultatrech = $Connect->query($Quer1);
		}
		$Quer ="SELECT  qtestock, nom_produit, num_produit, image, nom_fournisseur,type,volume, prixVente FROM produit NATURAL JOIN fournisseur NATURAL JOIN prix; ";
        $Resultatrech = $Connect->query($Quer);
										...
										<tr>
											...
											<th>
												<form class="form-inline" method="post" action="accueil.php">
													...
													<input type="hidden" name="num_produit" value="',$row['num_produit'],'"/>
